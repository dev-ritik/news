...
var pn_js=(function () {
    var moduleData = {},
        config ={
        'site_logo': '/photo/64095869.cms',
        'swjs_path' : '/sw.cms?v=4',
    
        'web_url' : 'https://timesofindia.indiatimes.com/',
        'web_source' : 'toiweb',
        ...
        'wap_url' : 'https://m.timesofindia.com/',
        'wap_source' : 'toiwap',
        ...
        'wap_block_cookie' : '_tointfcallow',
        ...
        'ga_allow_event' : 'Notification_Allow_New',
        'ga_block_event' : 'Notification_Block_New',
        'ga_ignore_event' : 'Ignore_New',
    ...
    function getAction(act){
        return (isMobile() === true) ? act + '_Wap' : act + '_Web';
    }
    ...
    function subscribe() {
        navigator.serviceWorker.ready.then(function(serviceWorkerRegistration) {
            serviceWorkerRegistration.pushManager.subscribe({
                ...
                //set status in pop up manager
                var notification_popup_callback = moduleData['notification_popup_callbacks'];
                notification_popup_callback && notification_popup_callback.onShowCallback(true);
        });
    }
            ...
            serviceWorkerRegistration.pushManager.getSubscription()
                .then(function(subscription) {
                    if (!subscription) {
                        //subscribe();
                        var notification_popup_callback = moduleData['notification_popup_callbacks'];
                        notification_popup_callback && notification_popup_callback.onReadyCallback(1);
                        return;
                    }
                })
                .catch(function(err) {
    ...
    function registerWithPopupManager(){
        moduleData['notification_popup_callbacks'] = TimesApps.PopUpManager.registerPopup("notification", subscribe);
    };
    ...
    function bindEvents(){
        if( window.TimesApps && TimesApps.PopUpManager ){
            ...
            require(['tiljs/event'], function(pubSub){
                pubSub.subscribe('popUpManagerLoaded', registerWithPopupManager);
            });
        }
    }
